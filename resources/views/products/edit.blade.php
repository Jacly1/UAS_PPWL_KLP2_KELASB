...
<div class="w-2/4 mx-auto">
    ...
    <form action="{{ route('store.edit', $product->id) }}" method="post" class="p-2">
        @csrf
        <div class="mx-5 mb-3">
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <textarea name="deskripsi" id="deskripsi" cols="30" rows="10" class="block p-2.5 w-full text-sm rounded-lg border bg-gray-700 border-gray-600 text-white focus:ring-blue-500 focus:border-blue-500" required>{{ $product->deskripsi }}</textarea>
                @error('deskripsi')
                <div>
                    <p class="text-red-500 text-base">{{ $message }}</p>
                </div>
            ...
            <div class="mb-3">
                ...
                <select name="status" id="status" class="block p-2.5 w-full text-sm rounded-lg border bg-gray-700 border-gray-600 text-white focus:ring-blue-500 focus:border-blue-500" required>
                    <option value="to do" {{ $product->status == 'to do' ? 'selected' : '' }}>To Do</option>
                    <option value="in process" {{ $product->status == 'in process' ? 'selected' : '' }}>In Process</option>
                    <option value="done" {{ $product->status == 'done' ? 'selected' : '' }}>Done</option>
                </select>
                @error('status')
                <div>
                    <p class="text-red-500 text-base">{{ $message }}</p>
            ...
            <div class="flex space-x-4 mt-5">
                <button type="submit" class="w-full text-center text-white text-lg font-bold tracking-wider uppercase bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 rounded-lg px-5 py-2.5 me-2 mb-2">
                    Edit
                </button>
                <a href="{{url('products')}}"class="w-full text-center text-lg font-bold tracking-wider uppercase bg-gray-600 hover:bg-gray-700 focus:ring-4 focus:outline-none focus:ring-gray-300 dark:focus:ring-gray-800 text-white rounded-lg px-5 py-2.5 me-2 mb-2">
                    Cancel
                </a>
            </div>
